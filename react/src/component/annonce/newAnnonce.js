...
function NewAnnonce(props) {
    ...
    const { register, control, handleSubmit, formState: { errors } } = useForm({defaultValues: {name_annonce: ''}});

    let newAnnonceForm = async () => {
        try {
            let res = await axios.post('http://127.0.0.1:8000/api/annonces/', {name_annonce}, {
                    "headers" : { "Authorization":"Bearer"+localStorage.getItem('access_token') }
                  });
            if (res.status === 200) {
                let tab = {};
                await Object.assign(tab, res.data.data);
                let data = update(props.newValue.data, {$push: [{id : tab.id, name_annonce: tab.name_annonce}]})
